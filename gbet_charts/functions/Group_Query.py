...
def env_query(attribute, start_time=0, end_time=datetime.now().timestamp()*1000):
    ...
    print(query)    
    mu = MongoUtil(DB)    
    return mu.aggregate(DB, OB_COL, query)


def dew_point_query(field, start_time, end_time):
    print("Two Variables", start_time, end_time)
    group_name = GBE_ID

    match = {"$match":{
        "status.status_qualifier":SUCCESS,
        # add field to criteria
    ...
    mu = MongoUtil()
    #print("Run Query")
    return mu.aggregate(DB, OB_COL, q)
   

def trial_env_query(field, attribute, start_date, end_date):
    # environmental data for an individual trial
    #start_date = session["defaults"]['trial']["start_date"]
    print("env_data", field, attribute, start_date, end_date)
    match = {"$match":{
           # need to add box id
           #"location.field.name":field,
           "status.status_qualifier":SUCCESS,
           "subject.attribute.name": attribute,
           "$and":[{"time.timestamp":{"$gt":start_date}},
...
def trial_by_week_query(attribute, school):
    ...
    # run the query
    mu = MongoUtil(GBE)
    return mu.aggregate(GBE, GBE_2020, q)
'''    
...
def box_test_query(attribute, week):
    # test of box chart
...
def histogram_chart_query(attribute, title="None", reduce=1):
...
def two_var_scatter_query(var1, var2, title):
...
def environmental_query(env_var, attr, title):
...
def full_env_query(attr, week, title):
